...
for row in range(len(code)):
    num = ''
    # for col in range(len(code[row])):
    #     print(code[row][col], end='')
    # print()
    for col in range(len(code[0])):
        if code[row][col] not in digit or col == len(code[0]) - 1:
            length = len(num)
            
            if code[row][col] in digit:
                num += code[row][col]
            ...
            if length > 0:
                ...
                for i in range(left, right):
                    if row > 0:
                        if code[top][i] not in digit and code[top][i] != '.':
                            part = True
                            symbol = code[top][i]
                            break
                    if row < len(code) - 1:
                        if code[bottom][i] not in digit and code[bottom][i] != '.':
                            part = True
                            symbol = code[bottom][i]
                            break
                    
                    if code[row][i] not in digit and code[row][i] != '.':
                        part = True
                        symbol = code[row][i]
                        break
                    
                # print(num + code[row][col], end='')
                if part:
                    ans += int(num)         
                    # print('^' + ' ' * (length - 1), end='')
                else:
