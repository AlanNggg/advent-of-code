...
for string in inputs:
    ...
    while i < len(string):
        if string[i] in digits:
            ...
            for digit_string in special_digits:
                length = i + len(digit_string)
                if length <= len(string) and string[i:length] == digit_string:
                    left = special_digits[digit_string]    
    ...
    while j >= 0:
        if string[j] in digits:
            ...
            for digit_string in special_digits:
                length = j + len(digit_string)
                if length <= len(string) and string[j:length] == digit_string:
                    right = special_digits[digit_string]   
        if right != -1:
            break
        else:
            j -= 1
...
inputs = '''Game 1: 4 red, 1 green, 15 blue; 6 green, 2 red, 10 blue; 7 blue, 6 green, 4 red; 12 blue, 10 green, 3 red
