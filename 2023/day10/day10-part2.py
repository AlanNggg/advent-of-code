...
inputs = '''FF--7-|7F--7F--|7F77-.FJF7F-7-F|7.77.FF7-F7---7-FJ7.|J--FJJ-FF-.JJ7LF7.J7F-7-F77L7.7-7.7FF77FF7..F--L7JF--.FF.7F|JFL7.7FL77.-7|7.F7-7FJ.FJ.-
...
L|L|.LLL-JL|.|-777-L77J.L-LL|7L|.L7J-L.FL-J.LJL7J-7J-L.LL-77.7-JFJL--|7L|J.--J.LJL-JJ.F7LJ.LJ.L----JLJJ.FJ|...7J-L-L7L|-7L7-LFJJ-|J--|7JLL-L'''.split('\n')
# inputs = '''..F7.
# .FJ|.
# SJ.L7
# |F--J
...
while open_list:
    ...
    while level_size != 0:
        ...
        tile = tiles_map[tiles[x][y]]
        cache[x][y] = level

        closed_list.append((x, y))
        if x - 1 >= 0 and 'top' in tile and tiles[x-1][y] in tile['top'] and (x - 1, y) not in closed_list:
            open_list.append((x-1, y))
        if x + 1 < row and 'bottom' in tile and tiles[x+1][y] in tile['bottom'] and (x + 1, y) not in closed_list:
            open_list.append((x+1, y))
        if y - 1 >= 0 and 'left' in tile and tiles[x][y-1] in tile['left'] and (x, y - 1) not in closed_list:
            open_list.append((x, y-1))
        if y + 1 < col and 'right' in tile and tiles[x][y+1] in tile['right'] and (x, y + 1) not in closed_list:
            open_list.append((x, y+1))
...
for i in range(row):
    for j in range(col):
        # print((i, j), end='')
        # print(f'{tiles[i][j]:2} ', end='')
        # print(f'{cache[i][j]:2} ', end='')
        steps.append(cache[i][j])
    # print()
ans = max(steps)
print(ans)
...
#     if x - 1 >= 0:
#         if 'top' in tile:
#             if tiles[x-1][y] in tile['top'] and (x - 1, y) not in closed_list:
#                 post_order((x-1, y))

#     if x + 1 < row:
#         if 'bottom' in tile:
#             if tiles[x+1][y] in tile['bottom'] and (x + 1, y) not in closed_list:
#                 post_order((x+1, y))
     
#     if y - 1 >= 0:
#         if 'left' in tile:
#             if tiles[x][y-1] in tile['left'] and (x, y - 1) not in closed_list:
#                 post_order((x, y-1))
#     if y + 1 < col:
#         if 'right' in tile and tiles[x][y+1] in tile['right'] and (x, y + 1) not in closed_list:
#             post_order((x, y+1))
